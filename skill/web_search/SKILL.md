...
When `artifact_paths` contains paths, those paths are relative to GraphiteUI's whitelisted skill artifact directory and can be opened by the Output viewer. Downstream agent nodes can receive the file state and read text documents into model context.
