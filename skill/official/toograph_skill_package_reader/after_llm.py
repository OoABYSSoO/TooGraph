...
def toograph_skill_package_reader(**skill_inputs: Any) -> dict[str, Any]:
    ...
    for scope, skill_dir in candidates:
        if skill_dir.is_dir():
            ...
            return {
                ...
                "result": f"Read {package['file_count']} files from {scope} skill '{skill_key}'.",
                "activity_events": [
                    {
                        "kind": "skill_package_read",
                        "summary": f"Read Skill package {skill_key}.",
                        "status": "succeeded",
                        "detail": {
                            "skill_key": skill_key,
                            "source_scope": scope,
    ...
    return _failure(f"Skill '{skill_key}' was not found in {scope_text} packages.")
...
def _candidate_roots(repo_root: Path, skill_key: str, requested_scope: str) -> list[tuple[str, Path]]:
    roots = {
        "user": repo_root / "skill" / "user" / skill_key,
        "official": repo_root / "skill" / "official" / skill_key,
    }
