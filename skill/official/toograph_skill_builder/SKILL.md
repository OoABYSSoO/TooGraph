...
- The generated `skill_json` must not include `required` on Skill IO fields; `stateInputSchema` only lists state fields the Skill truly needs from the graph.
...
- Do not include legacy fields such as `enabled`, `hidden`, `selectable`, `requiresApproval`, `capabilityPolicy`, `health`, `configured`, `healthy`, `targets`, `executionTargets`, `runPolicies`, `kind`, `mode`, `scope`, `label`, or Skill IO `required`.
