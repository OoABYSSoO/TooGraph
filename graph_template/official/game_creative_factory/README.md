...
2. 通过 `memory_recall` 只读召回品牌偏好、历史素材表现、禁用表达和审核反馈。
...
9. 通过 `memory_candidate_writer` 写入 candidate 状态的创意经验候选。
...
- `memory_recall`: 只读召回品牌偏好、历史素材表现、禁用表达和审核反馈。
- `memory_candidate_writer`: 只写 candidate 状态候选记忆，不自动应用为 active。
...
`eval_cases.json` 覆盖 game_genre 是否影响策略、多源输入是否被使用、竞品模式是否被总结、脚本变体是否差异化、分镜/提示词是否可执行、审核评分是否包含平台合规风险。
