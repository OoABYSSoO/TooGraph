...
| Game Creative Factory | 通用游戏广告创意生产 Agent | P1 | 保留为高级展示模板；SLG 只是 `game_genre` 示例 |
...
# 8. P1 模板二：Game Creative Factory

## 8.1 模板名称

英文名：`Game Creative Factory`

中文名：`通用游戏广告创意生产 Agent`

`game_genre` 是模板输入字段，SLG 只是一个示例值；同一模板应能按输入适配 RPG、策略、休闲、模拟、卡牌、塔防、解谜、射击等游戏类型。

## 8.2 一句话定位

> 将游戏广告创意生产流程拆成可观察的 Agent 工作流：游戏类型输入、新闻辅助、竞品素材分析、视频理解、模式总结、Creative Brief、多版本脚本、图片分镜、视频提示词、审核评分、返修循环和 Artifacts 输出。
...
Game Creative Factory：用户使用示例素材分析结果，`game_genre` 可填 `SLG`
```
...
Game Creative Factory：RSS + 素材抓取/上传 + 视频理解 + 审核返修，`game_genre` 控制题材口径
```
...
整理 Game Creative Factory 模板思路和示例数据，SLG 作为示例游戏类型输入
```
