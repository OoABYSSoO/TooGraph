...
- 低层 `activity_events` 已有第一阶段统一形状，覆盖 Skill 调用、权限暂停、动态子图调用、本地文件读取/列表/搜索/写入/脚本、本地文件夹输入上下文读取、脚本测试、`web_search` 下载和图补丁草案；运行详情已能聚合父图、子图、低层活动和权限暂停；Buddy Home 写回和图补丁应用/revision 仍未补齐。
