...
1. 文件列表/搜索活动事件：为本地文件夹列表、文件搜索或文件选择刷新补低层 `activity_events`，记录 root、query、结果数量、跳过项和失败原因；前端 run capsule 和运行详情能回放这些事件。
2. 澄清暂停协议核验：确认 `buddy_turn_intake` 的澄清等待由 `interrupt_after`、resume payload 和标准 `awaiting_human` 表达；补测试覆盖刷新后恢复、拒绝和取消，不再依赖缺必填 state 报错。
3. 暂停期间队列策略：明确暂停 run 收到新消息时的处理规则，区分“补充当前断点”和“开启新用户回合”；悬浮窗和伙伴页面使用同一套状态转换。
4. 伙伴页面确认视图：把悬浮窗已有的确认、拒绝、取消、补充输入和恢复能力扩展到伙伴页面，复用标准暂停卡片和 run 协议，不实现第二套确认 UI 状态机。
