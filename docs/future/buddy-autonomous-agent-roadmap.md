...
- 低层 `activity_events` 已有第一阶段统一形状，覆盖 Skill 调用、权限暂停、动态子图调用、本地文件/脚本、本地文件夹输入上下文读取、脚本测试、`web_search` 下载和图补丁草案；文件列表/搜索、Buddy Home 写回、图补丁应用/revision 和父子图运行详情聚合仍未补齐。
...
- 巩固当前 `buddy_autonomous_loop`：澄清暂停必须真实使用 `interrupt_after`，能力循环继续保持单 `capability` state 和单 `result_package` 输出。
- 扩展统一 `activity_events`：继续覆盖文件列表/搜索、Buddy Home 写回、图补丁应用/revision 和父子图运行详情聚合。
- 补暂停期间队列策略和伙伴页面确认视图：悬浮窗已有“执行当前方案 / 补充内容 / 拒绝 / 取消 / 刷新或重新打开会话后恢复”，伙伴页面也应复用同一套运行与确认体验。
- 完善父子图运行详情：父子事件统一时间线、`subgraph_path` 定位、动态子图断点展示、内部 artifact 和权限归属。
...
- 建 Buddy Home writer Skill，只负责受控读取、生成 diff、写入指定 Buddy Home 文件和返回 revision/artifact。
- 建 Buddy Home 写回模板：读取 `buddy_self_review` 的 `memory_update_plan` 与 `buddy_evolution_plan`，生成候选 diff，必要时请求用户审批，再通过 writer Skill 应用。
- 为 `AGENTS.md`、`SOUL.md`、`USER.md`、`MEMORY.md`、`policy.json`、会话摘要和报告写回建立统一 revision 记录。
- 保持记忆和复盘内容的权限边界：它们是上下文，不是系统指令，不能提升 Buddy 权限或覆盖更高优先级规则。
...
- 重建图编辑命令流：替代历史 `graph_patch.draft` stub，补齐 patch 预览、validator、GraphCommandBus-style apply、graph revision、undo/redo 和审计。
- 建“修改当前图”和“创建新模板/子图”的图模板流程：生成草案、校验、可选试跑、预览、审批、保存。
- 增强能力选择审计：记录候选数量、被拒绝候选简短原因、选中能力权限摘要、置信度或缺口说明。
- 继续巩固 `toograph_skill_creation_workflow`：能力缺口可以进入 Skill 创建、测试、审查和受控写入流程，不能变成隐藏 `self_evolve` Skill。
...
- 做上下文压缩和结果预算：长历史摘要、`result_package` 展开预算、大日志/大文件摘要策略、运行详情按需展开。
- 补能力调用和模型调用的耗时、token、artifact 大小、结果体积等统计，支撑后续优化判断。
- 试点图级并行执行：先做固定 fanout 和只读子图，不把 `capability` 改成列表，不让多个动态能力在同一 LLM 节点内竞争。
- 迁移内部 `agent` kind 命名到用户心智一致的 LLM 节点语义，同时保持 `node_system` 单一协议，不引入第二套节点格式。
