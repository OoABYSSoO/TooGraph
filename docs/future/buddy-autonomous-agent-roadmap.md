...
本文记录 TooGraph 必须长期遵守的架构约束、已经达到商业化基线的能力边界，以及后续完整模式扩展候选。当前事实以代码、官方模板 JSON、Action manifest 和测试为准；这里不再维护逐日实现快照。

反查入口：

- 官方图模板：`graph_template/official/*/template.json`
- 官方 Action：`action/official/*/action.json`
...
- 图才是 Agent。多步智能必须由 graph、Subgraph、Condition、Action 和 Output 表达，单个 LLM 节点只做一次模型调用、一次结构化输出或一次能力调用准备。
- `node_system` 是唯一正式图协议，`state_schema` 是节点输入输出的唯一数据来源。不要引入并行图格式、隐藏节点契约或产品特例通道。
- 一个 LLM 节点最多使用一个显式能力来源：无能力、一个 `config.actionKey` Action，或一个输入 `capability` state。
- Action 是一次受控能力调用。Action 不拥有多轮自治、最终回复生成、长期记忆策略、后续能力选择或 retry loop。
...
- 将前端 affordance 失配和重试失败归并到统一 failure category；Action 预检失败已经覆盖目标不存在、页面快照过期、权限阻止和输入绑定失败，auto-resume 结果已经覆盖目标 run 失败、用户中断和前端执行失败，前端缺失目标已归类为 `target_not_found`，带重试证据的前端执行错误已归类为 `frontend_retry_failed`。
...
- 扩展编辑已有图覆盖面：Graph Edit Playback 已支持移动节点、节点重命名/简介/Agent 任务说明、修改 input/output config、修改 state、选择 Agent Action、流程/条件路由连线、删除节点与同计划恢复节点；Buddy 页面操作已能发现并点击编辑器“保存为模板”动作；编辑器节点选择已支持单选、多选切换、键盘选择、当前选择 affordance 和多选清除入口。
- 历史 `graph_patch.draft` stub 已从 Buddy command 与前端 API surface 移除；图修改入口必须走编辑器语义命令、校验、revision 和撤销路径，后台复盘与记忆整理提示继续显式禁止该旧 action。
...
- 当前官方 selector 已收敛为固定页面操作入口，只输出 `found` 与 `capability`，其中 capability 固定为 `toograph_page_operation_workflow`；后续如恢复多能力发现，应重新通过图模板表达候选、校验和审计，而不是把复杂 catalog 逻辑塞回 selector Action。
...
目标：把 Buddy Home 的基础读写升级为以 `MEMORY.md` 为唯一长期记忆权威、以 `buddy.db` 为会话召回索引的可审计记忆系统。

当前基线与后续扩展：

- `MEMORY.md` 是唯一长期记忆正文，用户在 Buddy Home 中直接编辑；所有自动更新都通过 `memory_document.update` 写完整文档并留下可恢复 revision。
- `buddy.db` 只保存 Buddy 会话、消息、命令、revision、报告和会话召回索引；不再保存长期记忆表或候选记忆表。
- `buddy_session_recall` 只读 `buddy.db` 的真实会话消息，支持 browse、discover、scroll 三种模式，返回命中会话窗口，不写长期记忆。
- `buddy_home_writer` 负责低风险 Buddy Home 写回，包含 `memory_document.update`、报告、profile、policy 非权限字段和 capability stats；不生成逐条记忆审批流。
- Buddy 页面只展示 `MEMORY.md` 与变更历史；运行详情只通过 activity timeline 展示 `buddy_session_recall` 与 `buddy_home_write` 的审计摘要。
- 后续 Hybrid RAG 可以作为会话召回或外部知识增强出现，但不能重新引入并行的长期记忆权威。
...
- 为每个 LLM 节点建立上下文装配报告：agent 节点运行记录已在 `node_executions[].artifacts.context_assembly_report` 中记录 LLM phase、读取的 state、文件、result outputs、记忆文件、知识库 chunk、Action result 以及对应字符数/token 估算。
...
- 建立只读 fanout 的并行上下文装配：Buddy Home MEMORY.md、知识库检索、页面上下文压缩、能力候选读取可以并行，合并节点负责预算和冲突处理。
...
目标：让官方模板、业务模板、Buddy 主循环和关键 Action 可以持续回归验证。
...
业务模板用于证明 TooGraph 能承载真实工作流，而不是只展示单次问答。所有模板必须有多源输入、结构化中间 state、显式 Action/Subgraph、artifact 输出、审计和 Eval；每个模板都要支持 mock data 模式，保证无外部账号时也能演示。
...
- 优先选择能展示 Graph 编排、Action 调用、Knowledge/RAG、Memory、Human review、Eval、Artifacts、Revision loop 和 Citation trace 的模板。
...
- 官方模板落在 `graph_template/official/<template_id>/template.json`，并在模板 manifest 中声明 `template_id`、名称、描述、类别、输入 schema、输出 contract、可发现性、所需 Action/Subgraph 和权限说明。
...
- 模板 README 固定包含用途、目标用户、工作流、输入、输出、Required Actions/Subgraphs、Sample run、Eval cases、Safety notes 和 Customization。
...
- 每个模板声明输入 schema、Graph State、节点流程、Action 列表、权限说明、失败边界和最终 output contract。
- 需要用户补充信息时，通过最终输出询问并结束本轮；下一轮从普通输入和历史上下文读取补充，不使用模板断点做对话。
- 输出长期经验时，只生成记忆整理、证据和作用域建议，不直接绕过 Store、command、revision 或审批。
...
落地切片：轻量模式已落在 `graph_template/official/policy_navigator_agent/`，支持粘贴政策正文、PDF/Word 文本抽取结果、mock 政策文件、用户画像、可选知识库声明、只读 `buddy_session_recall`、citation map、权益卡片、保守资格初判、行动清单、不确定项和持久化最终 Markdown 输出；README、mock data、示例输出和 eval case metadata 已随模板包提交。完整模式候选扩展包括 URL/PDF 解析、多文件新旧冲突识别、真实知识库检索执行器接入和评测自动运行；这些不阻塞当前 Gallery 商业化基线。资格初判只能保守表达，不能输出法律、财务或行政审批承诺。
...
落地切片：轻量模式已落在 `graph_template/official/ai_news_digest_to_wechat_article/`，支持用户粘贴多条新闻链接/正文、mock 新闻材料、可选 `web_search` 联网补充开关、只读内容偏好 `buddy_session_recall`、清洗去重、主题聚类、Top 新闻卡片、公众号文章、事实检查、多平台分发包、artifact/output contract、README、mock data、示例输出和 eval case metadata。完整模式候选扩展包括 RSS 源、定时运行、主题订阅、真实用户选题偏好闭环、封面提示词和评测自动运行；这些不阻塞当前 Gallery 商业化基线。
...
关键节点：`style_profiler` 提取常用句式、段落长度、语气、举例方式、标题风格和表达偏好；`ai_tone_detector` 标记空泛总结、过度排比、总结腔、缺少具体例子和缺少个人判断；`human_like_rewriter` 按风格画像重写；最终包只标出可供后台记忆整理参考的稳定偏好。

落地切片：轻量模式已落在 `graph_template/official/multi_platform_content_repurposer/`，支持一篇源内容、目标平台列表、少量历史样本、传播目标、普通输入式人工反馈、只读风格 `buddy_session_recall`、核心观点提取、风格画像、平台策略、平台初稿、AI 味检测、风格重写、平台适配评分、发布计划、artifact/output contract、README、mock data、示例输出和 eval case metadata。完整模式候选扩展包括更细的用户反馈二次修改体验、平台分发评分、封面生成提示词和评测自动运行；这些不阻塞当前 Gallery 商业化基线。用户反馈应通过普通最终回复进入下一轮，不使用模板断点。
...
落地切片：轻量模式已落在 `graph_template/official/job_application_interview_coach/`，支持候选人画像、简历、项目经历、单个目标 JD、目标城市、薪资期望、可选模拟面试记录、只读 `buddy_session_recall`、JD 能力矩阵、经历匹配图、岗位匹配报告、差距分析、简历 bullet 改写、STAR/CAR 项目故事库、面试题预测、模拟面试反馈或评分表、阶段准备计划、薪资策略、artifact/output contract、README、mock data、示例输出和 eval case metadata。完整模式候选扩展包括多 JD 对比、模拟面试多轮记录、弱点趋势追踪、行业薪资数据和评测自动运行；这些不阻塞当前 Gallery 商业化基线。涉及职业建议时必须保留不确定项和人工判断提示。
...
落地切片：轻量模式已落在 `graph_template/official/game_creative_factory/`，支持 `game_genre`、游戏简报、目标用户、创意目标、市场/新闻材料、竞品广告素材分析、上传素材分析、平台约束、返修反馈、只读 `buddy_session_recall`、多源输入规范化、竞品模式总结、新闻辅助上下文、Creative Brief、脚本变体、图片分镜、视频提示词、创意审核评分、最佳变体选择、图片/视频生成任务清单、artifact/output contract、README、mock data、示例输出和 eval case metadata。完整模式候选扩展包括真实 RSS、广告素材抓取、视频理解服务、审核返修循环、生成任务下游执行和评测自动运行；这些不阻塞当前 Gallery 商业化基线。广告素材和平台合规风险必须显式输出。
...
落地切片：轻量模式已落在 `graph_template/official/product_competitor_research_agent/`，支持产品方向、竞品资料、用户评论、访谈材料、目标市场、研究目标、已有知识、只读 `buddy_session_recall`、证据索引、竞品画像、带证据的功能矩阵、反馈清洗和聚类、痛点提炼、机会点排序、MVP 方案、PRD 草稿、验证计划、artifact/output contract、README、mock data、示例输出和 eval case metadata。完整模式候选扩展包括 URL/截图/应用商店评论导入、知识库检索、真实批量证据导入和评测自动运行；这些不阻塞当前 Gallery 商业化基线。做浅了会退化成普通竞品分析，所以必须优先保证证据链和引用。
...
落地切片：轻量模式已落在 `graph_template/official/ecommerce_review_mining_agent/`，支持商品背景、自家评论、竞品评论、店铺反馈、目标平台、营销目标、合规说明、只读 `buddy_session_recall`、评论证据索引、去噪去重、自家/竞品/店铺反馈区分、好评点聚类、差评点聚类、用户画像、购买动机、证据支持的卖点和风险点、详情页文案、短视频脚本、小红书笔记、artifact/output contract、README、mock data、示例输出和 eval case metadata。完整模式候选扩展包括真实 CSV/JSON 批量导入、平台合规词库、评论来源统计、生成素材下游执行和评测自动运行；这些不阻塞当前 Gallery 商业化基线。
...
- 桌面端、Docker 和单机源码部署路径已统一到 `npm start` / `node scripts/start.mjs`；`docs/deployment.md` 记录源码运行、Docker、数据卷、端口绑定、健康检查和更新流程，Docker 镜像通过 `TOOGRAPH_HOST=0.0.0.0` 暴露单端口服务。
- 关键 UI 流程已有 Playwright 覆盖：Buddy 虚拟操作跟随/后台证据、知识库管理、Gallery 多语言与模板打开、运行记录恢复、权限确认恢复，以及真实编辑器模板运行。
- 知识库管理已覆盖官方导入、检索、引用展示、重建索引、删除确认、删除清理和检索质量评测。
- 内部 `agent` kind 的用户心智迁移为 LLM 节点语义是长期命名清理方向；不能引入第二套协议。

## 不再维护的方向

- 不恢复 `docs/current_project_status.md` 这类当前实现快照。
- 不恢复 `docs/future/toograph_p0_p1_development_goals.md` 或平行的模板库长期规划。
- 不创建单体 `self_evolve` Action 或 Buddy 专用隐藏 runtime。
