...
- 第一阶段可见运行闭环已完成：低层 `activity_events` 覆盖 Skill 调用、能力选择审计、权限暂停、动态子图调用、本地文件读取/列表/搜索/写入/脚本、本地文件夹输入上下文读取、脚本测试、`web_search` 下载和图补丁草案；运行详情已能聚合父图、子图、低层活动、artifact badge 和权限暂停。
...
4. 写回结果展示：在 run capsule、运行详情和相关 Buddy Home 视图里展示写回 diff、revision、来源 run、跳过原因和失败原因。
5. 版本切换体验：在 Buddy 页面 History 中补齐 diff 预览、目标筛选、来源 run 展示和清晰的恢复/切换操作。
