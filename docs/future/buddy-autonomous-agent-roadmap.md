...
- 低层 `activity_events` 已有第一阶段统一形状，覆盖 Skill 调用、权限暂停、动态子图调用、本地文件/脚本、`web_search` 下载和图补丁草案；文件探索/搜索、Buddy Home 写回、图补丁应用/revision、脚本测试细节和父子图运行详情聚合仍未补齐。
...
2. 扩展统一 `activity_events`：继续覆盖文件探索/搜索、Buddy Home 写回、图补丁应用/revision、脚本测试细节和父子图运行详情聚合。
