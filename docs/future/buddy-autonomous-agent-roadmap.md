...
- 伙伴浮窗已有每条助手消息自带的运行过程胶囊、节点级流式输出预览、每步耗时、完成后折叠摘要、请求理解阶段的 `visible_reply` 上下文、正式 root output 回复和后台复盘解耦。
...
- Buddy Home 自主写回基础链路已完成：显式后台模板、受控 writer Skill、command、revision、activity event、运行详情展示和 Buddy Home 历史前后值/来源 run 展示已经落地；Buddy 页面 History 已支持按目标筛选、字段级紧凑 diff、来源 run/command 和 revision 恢复；后续继续补更细的非权限 policy、报告和能力统计写回规则。
...
当前正确边界是：用户不确认每次 Buddy Home 是否应该更新；是否更新由模型在自主复盘图中判断。低风险 memory、session summary 和 profile 写回自动通过 `buddy_home_writer` 应用并留下 command、revision 和 activity event；权限升级、能力边界扩大、任意文件写入、脚本执行、图补丁和 revision restore 不属于自动写回范围。
...
- 每条伙伴助手消息都有配对 run capsule，能展示父图公开输出、过程回应、能力调用、权限暂停、错误和低层活动摘要。
...
6. 写回范围扩展：在保持不可提权和可恢复边界的前提下，继续细化 policy 非权限偏好、报告和能力使用统计的写回规则。
