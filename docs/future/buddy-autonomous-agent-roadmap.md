...
- Buddy Home 自主写回基础链路已完成：显式后台模板、受控 writer Skill、command、revision、activity event、运行详情展示和 Buddy Home 历史前后值/来源 run 展示已经落地；Buddy 页面 History 已支持按目标筛选和字段级紧凑 diff；后续继续补更清晰的版本切换体验和更细的非权限 policy/报告类写回规则。
...
5. 版本切换体验：Buddy 页面 History 已支持按目标筛选和字段级紧凑 diff；后续继续补齐更清晰的恢复/切换操作。
