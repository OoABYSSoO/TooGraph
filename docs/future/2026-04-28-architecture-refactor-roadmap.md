...
- `core/langgraph/runtime_setup.py` 已承接 LangGraph state preparation、state schema/reducer construction、sentinel endpoint mapping、input boundary status marking、after-breakpoint passthrough callable setup、execution-edge index construction 和 after-breakpoint compile setup；`core/langgraph/runtime.py` 继续保留图构建、node/route callable、checkpoint、interrupt、cycle、progress persistence 和 output boundary execution。
- `core/langgraph/progress.py` 已承接 LangGraph progress persistence wrapper，保证 checkpoint metadata sync 先于 run progress persistence；`core/langgraph/runtime.py` 继续决定何时在 graph 初始化、node callable、route callable 和失败路径触发持久化。
- `core/langgraph/finalization.py` 已承接 completed/failed run finalization，包括 output boundary collection、cycle summary、checkpoint metadata sync、artifact refresh、snapshot append、save 和 run event publication；`core/langgraph/runtime.py` 继续保留完成/失败路径判断，并显式传入 patch-sensitive 的 `save_run`。
- Phase 140 后 `core/langgraph/runtime.py` 从约 1,040 行降到 458 行；execution callables 和图构建主流程仍留在 runtime，checkpoint、interrupt、cycle、setup、progress 和 finalization helper 已迁出。
...
- 2026-04-30：`useCanvasPinchZoomState.ts` 已承接 active touch pointer cache、pinch zoom ref、pinch start/update/release 和 touch-move scheduled update；`EditorCanvas.vue` 从 3319 行降到 3251 行，并继续保留 DOM event side effects、panning lifecycle、viewport `zoomAt`、connection pointer-up 和 node drag/resize release。
