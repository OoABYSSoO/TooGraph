...
- Phase 104 后 `node_system_executor.py` 从 1,226 行降到 653 行；执行主流程、副作用、output boundary collection、streaming delta 和 node handler 仍留在 executor，后续应继续优先抽取 output boundary collection、input boundary coercion 或 node handler。
...
当前 P4 进展：`model_provider_client.py` 的共享 HTTP/request 层、provider discovery 层、OpenAI-compatible chat transport、Anthropic messages transport、Gemini generate-content transport、Codex responses transport 和共享 response parsing 已完成抽取；`node_system_executor.py` 的 condition evaluation、agent prompt、LLM output parser、execution graph、state I/O、output artifact 和 run artifact helper 已完成抽取；LangGraph runtime 仍待迁移。
