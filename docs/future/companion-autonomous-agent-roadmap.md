...
- 当前官方 Skill 包包括 `web_search`、`graphiteui_capability_selector` 和 `graphiteUI_skill_builder`。
...
- 偏离新职责的旧 `create_user_skill` 内置模板已删除。新的 `graphiteUI_skill_builder` 只产出 Skill 包文件内容，完整用户 Skill 生成流程仍需要补齐确认、写入、测试、修复和启用节点。
...
- 围绕 `graphiteUI_skill_builder` 补齐用户 Skill 生成图流程：从需求澄清、示例确认、文件内容生成，到受控写入、测试、错误修复和启用。
