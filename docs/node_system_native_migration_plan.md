...
- 节点显示名、节点描述、state 名称和 state 内容的编辑已直接落到 `canonicalGraph`
- 新增输入/输出端口已走“选已有 state / 新建 state”主链，并直接修改正式协议
- 已绑定端口改名会直接反写对应 `state.name`
...
- `/api/templates` 和 `/api/templates/{id}` 已直接返回正式模板协议
- `/api/graphs` 和 `/api/graphs/{graph_id}` 已直接返回正式 graph 协议
