...
- Graphite 原生 Python 常量定义已移除；外部 skill 源按各自官方目录发现，GraphiteUI 运行时只使用已导入到 `.graphite/skills/` 的本地 skill 仓库
- GraphiteUI 现已建立自己的本地 skill 仓库 `.graphite/skills/`，外部 skill 源只负责发现与导入，不允许直接管理外部源文件
