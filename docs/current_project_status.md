...
- 当前官方技能包包括 `web_search`、`graphiteui_capability_selector` 和 `graphiteUI_skill_builder`。后续新能力应按当前统一 Skill 结构专门编写。
...
创建用户自定义 Skill 的旧官方模板已删除。当前由 `graphiteUI_skill_builder` 承担窄职责的 Skill 生成能力：只根据需求产出 `skill.json`、`SKILL.md` 以及必要的 `before_llm.py` / `after_llm.py` 文件内容，写入、测试、修复和回滚不由该 Skill 执行。
...
- 为 `graphiteUI_skill_builder` 补充后续图流程：用户确认、写入用户 Skill 目录、运行 smoke test、基于错误再次调用 builder 或修复节点，以及最终启用。
- 完成更完整的图运行展示。
