...
- 内部来源：主循环中的两个 Subgraph 节点分别来自 internal 模板 `buddy_request_intake` 和 `buddy_capability_loop`。当前协议仍要求 Subgraph 节点嵌入 `config.graph`，因此主循环保存的是模板内容副本；契约测试会校验这些嵌入副本与 internal 模板保持一致。官方 JSON 模板就是当前事实来源，维护时直接修改对应模板 JSON 及其主循环嵌入副本，不再保留额外的 Buddy 模板重建脚本。
