...
- `subgraph` 已是正式节点类型：可从已保存整张图创建实例，运行时隔离内部 state，公开 input/output 映射为父图端口，并可双击打开当前实例编辑窗口。
...
- subgraph 节点把一张已保存图复制为当前父图内的独立实例。节点卡片展示公开输入/输出胶囊、内部缩略图和内部能力摘要；双击节点会打开当前实例编辑窗口，窗口内复用正式画布编辑器，保存时只回写该节点的 `config.graph`。
