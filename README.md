...
- 当前官方 Skill 包包括 `web_search`、`local_workspace_executor` 和 `graphiteUI_skill_builder`。
- `web_search` 支持联网搜索、引用整理、可选网页正文抓取、本地 source document artifact 输出，并把原文网址写入下载后的本地文档。
- `local_workspace_executor` 支持受策略约束的本地文件读写、目录操作和命令/脚本运行；默认写入范围限制在 `backend/data` 下，越权时返回 blocked 结果和建议白名单。
- `graphiteUI_skill_builder` 支持创建、校验、测试、修订和回滚用户自定义 Skill 包，只写入 `backend/data/skills/user`，官方 Skill 只读。
...
| `GET /api/local-executor-policy` | 查看本地执行器权限策略 |
| `POST /api/local-executor-policy/allow-root` | 为本地执行器追加受控读写/执行白名单 |
...
- `create_user_skill.json`（界面名称：创建自定义技能）

`advanced_web_research_loop.json` 是当前联网搜索基线模板：它会规划搜索词、调用 `web_search`、判断证据是否足够、按需循环补查，并分别输出最终答案、证据链接和本地 source documents。

`create_user_skill.json` 是当前用户自定义 Skill 创建模板：它会先检查现有技能是否已满足需求，再通过断点向用户澄清需求、确认示例输入输出和设计方案，随后调用 `graphiteUI_skill_builder` 写入用户 Skill、运行 smoke test，并在测试失败时进入有限次数的自动修复循环。
