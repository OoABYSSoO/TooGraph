...
- Skill 定义来自 `skill/<skill_key>/` 文件夹；当前内置 `web_search`、`web_media_downloader` 和 `game_ad_research_collector`。
- `web_search` 支持联网搜索、引用整理、可选网页正文抓取、本地 source document artifact 输出。
- `web_media_downloader` 支持从公开或用户授权的网页/直链下载图片、视频、音频等媒体，并返回本地 artifact 路径。
- `game_ad_research_collector` 支持采集游戏行业 RSS 上下文、发现公开视频广告素材、下载视频 artifact，并服务于广告创意分析流程。
