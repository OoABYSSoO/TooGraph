...
GraphiteUI 的推荐模型配置入口是 Model Providers 页面。你可以在界面里配置本地模型网关、私有 OpenAI-compatible 网关或云端 Provider，并选择默认文本模型；这些配置写入本机设置目录，不进入 Git。

本地或私有网关的常规流程：

1. 启动你要使用的 OpenAI-compatible gateway。
2. 打开 GraphiteUI 的 Model Providers 页面。
...
如果系统里的 Python 命令是 `python3`，用 `python3 --version` 检查即可。GraphiteUI 也支持通过 `PYTHON` 环境变量指定 Python 可执行文件路径。

如果需要运行 LLM 节点，还需要准备一个可访问的模型服务：可以是本地 OpenAI-compatible 网关、私有网关，或云端 Provider。模型入口在 GraphiteUI 的 Model Providers 页面配置，不通过启动环境变量配置。
...
镜像源只影响依赖下载；GraphiteUI 的启动方式仍然是 `npm start`。

### 配置模型入口

如果需要运行 LLM 节点，先启动本地模型网关或准备云端 Provider 凭据，然后在 GraphiteUI 的 Model Providers 页面完成配置。只查看 UI、编辑 graph 或浏览已有运行记录时，可以先不配置模型入口。
...
- `graphiteui_skill_creation_workflow`（界面名称：创建自定义 Skill）
