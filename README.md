...
- 本地数据：graph、preset、run、settings、Action state 和 checkpoint 写入 `backend/data/`；knowledge base 使用 SQLite + FTS。
...
- Knowledge base 可以通过 input 节点进入 graph。需要显式检索时，应通过 Action 或图模板读取知识库，不再依赖旧内置隐式检索能力。
...
- 当前官方 Action 包包括 `web_search`、`toograph_capability_selector`、`toograph_context_fanout`、`toograph_page_operator`、`toograph_action_builder`、`toograph_action_package_reader`、`toograph_graph_template_reader`、`toograph_graph_template_validator`、`toograph_graph_template_writer`、`toograph_script_tester`、`local_workspace_executor`、`buddy_session_recall`、`buddy_home_writer` 和内部遗留 `buddy_visible_subgraph_result_adapter`。
...
- `local_workspace_executor` 会在 Action 入参规划前预读节点任务或 state 中提到的仓库文件；运行时支持 `read` / `list` / `search` / `write` / `execute`，并对 `.git`、`.env`、`backend/data/settings` 做硬拒绝；它涉及本地写入和进程执行，是否暂停确认由当前图或 Buddy 的权限模式决定。
...
├── backend/
│   ├── app/
│   │   ├── api/              # graphs / runs / templates / presets / settings / actions / tools / knowledge / buddy
...
| `GET /api/actions/definitions` | 列出可用 Actions |
| `GET /api/actions/catalog` | 列出 Action catalog |
| `GET /api/tools/catalog` | 列出 Tool catalog |
...
`toograph_action_creation_workflow` 是创建用户自定义 Action 的官方流程模板：它会澄清需求、读取可选的已有 Action 包、生成示例输入输出，调用 `toograph_action_builder` 生成 `action_key` / `action.json` / `ACTION.md` / `before_llm.py` / `after_llm.py` / `requirements.txt`，再用 `toograph_script_tester` 测试生命周期脚本或相关文件；测试失败会回到构建节点修复，最后通过受控执行器写入 `action/user/<action_key>/`。
