...
后端仍保留 `LOCAL_BASE_URL`、`LOCAL_API_KEY`、`LOCAL_TEXT_MODEL` 及少量旧别名作为无界面配置、脚本化启动或历史兼容兜底。正常使用时不要把它们当作主配置方式；base URL 和 API key 会优先使用已保存的 Model Providers 配置，模型名应通过页面发现或保存，避免同时设置 `LOCAL_TEXT_MODEL` 干扰旧 local route 兜底路径。
