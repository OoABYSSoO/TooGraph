...
- 当前官方 Skill 包包括 `web_search`、`local_workspace_executor` 和 `graphiteui_capability_selector`。
...
- `graphiteui_capability_selector` 支持从当前启用的图模板和 Skill 清单中选择一个可用能力，输出单个 `capability` state。
...
创建用户自定义 Skill 的旧官方模板已删除，后续会按新的 Skill 生成职责重新设计。
