...
- `agent`：当前协议里的 LLM 节点，配置任务说明、模型、thinking、一个显式 Action 或一个动态 `capability` 输入，并读取或写入 state。
...
- `capability` state 表示一个互斥能力选择，`kind` 只能是 `action`、`subgraph`、`tool` 或 `none`。
- 动态能力执行节点必须把结果写入唯一 `result_package` state。

## Actions 与知识库

- Action 是显式挂在 LLM 节点上的，不是隐藏能力。
- 一个 LLM 节点最多使用一个显式能力来源。
- Action 的运行资源应放在 `action/official/<action_key>/` 或 `action/user/<action_key>/` 文件夹内，并通过 `action.json` manifest 暴露给 LLM 节点。
- 当前官方 Action 包括 `web_search`、`toograph_capability_selector`、`toograph_context_fanout`、`toograph_page_operator`、`toograph_action_builder`、`toograph_script_tester`、`local_workspace_executor`、`buddy_session_recall` 和内部 `buddy_home_writer`。
- knowledge base input 可以作为 state 进入图，但不会再自动挂载旧的内置检索能力。
