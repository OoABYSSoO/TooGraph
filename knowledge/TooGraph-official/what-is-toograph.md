...
- `agent`：当前协议里的 LLM 节点，定义任务说明、输入输出端口、模型配置、一个显式 Action 或一个动态能力输入。
...
- 你可以看到运行历史、节点执行结果、Action 输出、状态快照、warnings、errors 和本地 artifacts。
- 你可以把联网搜索、能力选择、Action 生成、脚本测试、本地文件操作这类能力做成显式 Action，由 graph 负责决定何时调用、如何审计和如何把结果传给下游。
