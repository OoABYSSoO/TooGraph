...
- `toograph_capability_selector`：根据当前 LLM 节点可见的普通图 state 先判断是否仍需能力；需要时从候选 Action、Subgraph、Tool 中选择并校验一个 `capability`，不需要时返回 `none`。`stateInputSchema.current_requirement` 只是连接提示，不是强制输入。
