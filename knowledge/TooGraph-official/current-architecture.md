...
- `buddy_visible_subgraph_result_adapter`：内部遗留 Action，把可见运行图模板得到的公开结果包装回 Buddy 能力循环使用的 `result_package`。
...
- `buddy_capability_loop`：Buddy 官方能力循环子图，默认不被能力发现，避免递归选择自己。
