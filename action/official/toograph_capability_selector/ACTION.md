...
- 目录内部为每个能力生成标准权限画像 `permissionTier` / `permissionProfile`，用于权限策略过滤、评分和 trace；LLM 上下文仍保持紧凑，不直接展开权限清单。
...
- 当运行时上下文提供权限策略时，`after_llm.py` 会拒绝不符合策略的能力，并在 trace 中写入 `permission_tier_not_allowed`。
- trace 的 `permission_summary` 使用同一标准权限画像，说明权限层级、是否需要审批和策略触发原因。
