...
def toograph_skill_builder(**skill_inputs: Any) -> dict[str, Any]:
    ...
    if skill_key:
        skill_json.pop("skillKey", None)
        skill_json.pop("skill_key", None)
        skill_json.pop("action_key", None)
        skill_json["actionKey"] = skill_key
    return {
        "skill_key": skill_key,
...
def _normalize_skill_key(value: Any, skill_json: dict[str, Any]) -> str:
    ...
    return _strip_markdown_fence(
        _as_text(
            skill_json.get("actionKey")
            or skill_json.get("action_key")
            or skill_json.get("skillKey")
            or skill_json.get("skill_key")
        )
    ).strip()
