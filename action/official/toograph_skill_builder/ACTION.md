...
- `after_llm.py` normalizes the generated identity and file-content fields, syncs `skill_json.actionKey` to `skill_key`, and strips accidental Markdown fences.

State inputs:

- `confirmed_skill_requirement`: confirmed requirement, boundaries, examples, and acceptance criteria for the target Action.

LLM output:

- `skill_key`: stable machine identifier for the generated Action, used later as the user Action directory name.
- `skill_json`: `action.json` as a JSON object using `toograph.action/v1`.
- `skill_md`: complete `ACTION.md` Markdown content.
...
Rules:
- `skill_key` must match `skill_json.actionKey`.
...
- The generated `skill_json` must not include `required` on Action IO fields; `stateInputSchema` only lists state fields the Action truly needs from the graph.
...
- The generated `skill_json` must not include local usage settings. Action visibility is controlled only by `action/settings.json` with an `enabled` flag, which the app generates outside the Action package.
