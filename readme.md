...
- The next editor should be implemented against the current backend system, not the demo file.
...
- `canvas-first` editor 第一阶段布局：浮动面板、可折叠 `State Panel`、自动收起的节点检查面板
...
- 当前 editor 节点库已支持搜索
- 当前模板注册表已有第二个模板：`hello_world`
- `hello_world` 模板用于验证前端编排到本地 OpenAI-compatible 模型服务的调用链
- 本地模型相关环境变量优先读取：
  - `LOCAL_BASE_URL` / `OPENAI_BASE_URL`
  - `LOCAL_API_KEY` / `OPENAI_API_KEY`
  - `LOCAL_TEXT_MODEL` / `TEXT_MODEL`

在 GraphiteUI 中，用户可以：

- 在 **Workspace** 中查看最近 graph、最近 runs 和系统状态
- 在 **Editor 入口页** 中先选择模板或已有 graph
