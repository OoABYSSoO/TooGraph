...
- Except for graph templates themselves, buddy long-term editable data should be organized under root `buddy_home/` as the Buddy Home. This directory is generated by the program when missing and is not tracked by Git. It includes profile, policy, memories, summaries, revisions, command logs, evolution reports, and capability usage notes. Official templates, official skills, and user skill packages keep their own established locations.
