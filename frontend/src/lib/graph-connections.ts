...
export function canConnectStateBinding(
  ...
  const isVirtualOutputSource =
    ...
    (sourceNode.kind === "agent" || sourceNode.kind === "batch" || sourceNode.kind === "subgraph" || (sourceNode.kind === "input" && sourceNode.writes.length === 0));
  ...
  if (isVirtualOutputSource) {
    ...
    if (isCreateAgentInputStateKey(targetStateKey)) {
      return targetNode.kind === "agent" || targetNode.kind === "batch" || targetNode.kind === "subgraph";
    }
    if (isVirtualAnyInputStateKey(targetStateKey)) {
      return targetNode.kind === "agent" || targetNode.kind === "batch" || targetNode.kind === "subgraph" || targetNode.reads.length === 0;
    }
  ...
  if (isCreateAgentInputStateKey(targetStateKey)) {
    return (targetNode.kind === "agent" || targetNode.kind === "batch" || targetNode.kind === "subgraph") && !targetNode.reads.some((binding) => binding.state === sourceStateKey);
  }
...
export function canConnectFlowNodes(
  ...
  if ((sourceNode.kind !== "input" && sourceNode.kind !== "agent" && sourceNode.kind !== "batch" && sourceNode.kind !== "subgraph") || !canNodeAcceptFlowTarget(sourceNode, targetNode)) {
    return false;
  }
...
export function canConnectStateInputSource(
  ...
  } else if (sourceNode.kind !== "agent" && sourceNode.kind !== "batch" && sourceNode.kind !== "subgraph") {
    return false;
  }

  if (isCreateAgentInputStateKey(targetStateKey)) {
    if (targetNode.kind !== "agent" && targetNode.kind !== "batch" && targetNode.kind !== "subgraph") {
      return false;
    }
  } else if (isVirtualAnyInputStateKey(targetStateKey)) {
    if (targetNode.kind !== "agent" && targetNode.kind !== "batch" && targetNode.kind !== "subgraph" && targetNode.reads.length > 0) {
      return false;
    }
