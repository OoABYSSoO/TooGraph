...
test("resolvePrimaryNavigationSection keeps graph editor detail routes inside the editor nav section", () => {
  ...
  assert.equal(resolvePrimaryNavigationSection("/knowledge/policy"), "knowledge");
  assert.equal(resolvePrimaryNavigationSection("/workspace"), "localFiles");
  assert.equal(resolvePrimaryNavigationSection("/workspace/policy"), "localFiles");
