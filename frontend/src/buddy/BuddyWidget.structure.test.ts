...
test("BuddyWidget renders a draggable virtual cursor and makes the mascot follow it", () => {
  ...
  assert.match(componentSource, /const BUDDY_VIRTUAL_CURSOR_MAX_FLIGHT_DURATION_MS = 360;/);
  ...
  assert.match(componentSource, /function moveVirtualCursorTo\(nextPosition: BuddyPosition, options: \{ updateAngle\?: boolean; durationMs\?: number; rotateDurationMs\?: number \} = \{\}\)/);
  ...
  assert.match(componentSource, /setVirtualCursorRotateTransitionDuration\(options\.rotateDurationMs \?\? BUDDY_VIRTUAL_CURSOR_ROTATE_TRANSITION_MS\);/);
  assert.match(componentSource, /if \(options\.updateAngle !== false\) \{[\s\S]*virtualCursorAngleDeg\.value = resolveContinuousVirtualCursorAngle\(resolveVirtualCursorFlightAngle\(currentPosition, nextPosition\)\);[\s\S]*\}/);
  ...
  assert.match(componentSource, /buddyMascotMotionConfig\.value\.virtualCursorFlightSpeedPxPerMs/);
  ...
  assert.match(extractCssBlock(".buddy-widget__virtual-cursor"), /translate var\(--buddy-widget-virtual-cursor-move-duration-ms,\s*180ms\) cubic-bezier\(0\.16,\s*1,\s*0\.3,\s*1\)/);
...
test("BuddyWidget keeps virtual-cursor follow jumps from being flattened by slow cursor drags", () => {
  ...
  assert.match(componentSource, /moveVirtualCursorTo\(resolveVirtualCursorOrbitPosition\(angle\), \{ durationMs: 0, rotateDurationMs: 0 \}\);/);
  ...
  assert.match(componentSource, /Math\.sin\(angle \* 2\)/);
