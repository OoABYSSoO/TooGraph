...
test("BuddyWidget executes virtual UI operation events through the virtual cursor", () => {
  ...
  assert.match(componentSource, /await resolveGraphEditPlaybackStepElementWithRetry\(step, playbackState\)/);
  assert.match(componentSource, /executeGraphEditPlaybackDragStep\(step, targetElement, playbackState\)/);
  assert.match(componentSource, /function resolveGraphEditPlaybackStepElementWithRetry\(step: GraphEditPlaybackStep, playbackState: GraphEditPlaybackUiState\)/);
