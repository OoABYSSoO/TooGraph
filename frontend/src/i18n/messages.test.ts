...
test("i18n messages expose knowledge operation progress and recovery labels", () => {
  const zh = messages["zh-CN"].knowledge;
  assert.equal(zh.indexingProgress, "索引进度 {percent}%");
  assert.equal(zh.nextRetryAt, "下次重试：{value}");
  assert.equal(zh.retryNow, "立即重试");
  assert.equal(zh.pauseIndexing, "暂停索引");
  assert.equal(zh.resumeIndexing, "继续索引");
  assert.equal(zh.openModelSettings, "打开模型设置");
  assert.equal(zh.operationStatus, "操作状态");
  assert.equal(zh.operationStage, "当前阶段");
  assert.equal(zh.operationId, "操作 ID");
  assert.equal(zh.completedJobs, "已完成");
  assert.equal(zh.pendingJobs, "待处理");
  assert.equal(zh.runningJobs, "运行中");
  ...
  assert.equal(zh.modelSettingsHint, "如果 embedding 模型或 provider 出错，请先检查模型供应商配置。");
  assert.equal(zh.statusIndexing, "索引中");
  assert.equal(zh.statusPartiallyReady, "部分可用");
  assert.equal(zh.statusReady, "已就绪");
  assert.equal(zh.statusEmpty, "空知识库");
  assert.equal(zh.statusFailed, "索引失败");
  assert.equal(zh.statusPaused, "已暂停");
  assert.equal(zh.statusPausedRetrying, "等待重试");
  assert.equal(zh.statusNeedsAttention, "需要处理");
  assert.equal(zh.lastIndexingError, "最后索引错误");

  const en = messages["en-US"].knowledge;
  assert.equal(en.indexingProgress, "Indexing progress {percent}%");
  assert.equal(en.nextRetryAt, "Next retry: {value}");
  assert.equal(en.retryNow, "Retry now");
  assert.equal(en.pauseIndexing, "Pause indexing");
  assert.equal(en.resumeIndexing, "Resume indexing");
  assert.equal(en.openModelSettings, "Open model settings");
  assert.equal(en.operationStatus, "Operation status");
  assert.equal(en.operationStage, "Operation stage");
  assert.equal(en.operationId, "Operation ID");
  assert.equal(en.completedJobs, "Completed");
  assert.equal(en.pendingJobs, "Pending");
  assert.equal(en.runningJobs, "Running");
  ...
  assert.equal(en.modelSettingsHint, "If the embedding model or provider is failing, check Model Providers first.");
  assert.equal(en.statusIndexing, "Indexing");
  assert.equal(en.statusPartiallyReady, "Partially ready");
  assert.equal(en.statusReady, "Ready");
  assert.equal(en.statusEmpty, "Empty");
  assert.equal(en.statusFailed, "Indexing failed");
  assert.equal(en.statusPaused, "Paused");
  assert.equal(en.statusPausedRetrying, "Waiting to retry");
  assert.equal(en.statusNeedsAttention, "Needs attention");
  assert.equal(en.lastIndexingError, "Last indexing error");
});
