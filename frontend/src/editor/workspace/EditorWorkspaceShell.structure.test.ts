...
test("EditorWorkspaceShell delegates run event stream parsing and URL projection to the shared model", () => {
  assert.match(componentSource, /import \{[\s\S]*buildRunEventStreamUrl,[\s\S]*listRunEventOutputKeys,[\s\S]*parseRunEventPayloadData,[\s\S]*resolveRunEventNodeId,[\s\S]*resolveRunEventText,[\s\S]*shouldPollRunStatus[\s\S]*\} from "@\/lib\/run-event-stream";/);
  ...
  assert.match(componentSource, /const text = resolveRunEventText\(payload\);/);
