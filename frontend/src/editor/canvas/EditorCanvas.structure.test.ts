...
test("EditorCanvas exposes page zoom controls and emits viewport draft updates", () => {
  ...
  assert.match(componentSource, /import \{[\s\S]*resolveCanvasPanPointerMoveAction,[\s\S]*resolveCanvasWheelZoomRequest,[\s\S]*resolveCanvasWorldPoint,[\s\S]*resolveCanvasZoomButtonAction,[\s\S]*type CanvasZoomButtonControl,[\s\S]*\} from "\.\/canvasViewportInteractionModel";/);
