...
test("buildSubgraphMiniMapLayout routes sequence edges from right output to left input frames", () => {
  ...
  assert.equal(layout.edges[0]?.path ?? "", "M 162 43 H 204");
  assert.equal(layout.edges[1]?.path ?? "", "M 518 43 H 536 V 123 H 26");
});
