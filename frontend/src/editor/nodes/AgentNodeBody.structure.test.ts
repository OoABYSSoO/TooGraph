...
test("AgentNodeBody owns agent body presentation and forwards parent side effects", () => {
  ...
  assert.match(componentSource, /defineProps<\{[\s\S]*nodeId: string;[\s\S]*body: AgentBodyViewModel;[\s\S]*orderedInputPorts: NodePortViewModel\[\];[\s\S]*orderedOutputPorts: NodePortViewModel\[\];[\s\S]*modelOptions: AgentModelOption\[\];[\s\S]*availableSkillDefinitions: SkillDefinition\[\];[\s\S]*attachedSkillBadges: AttachedSkillBadge\[\];[\s\S]*\}>/);
  assert.match(componentSource, /defineEmits<\{[\s\S]*\(event: "port-click", anchorId: string, stateKey: string\): void;[\s\S]*\(event: "update:model-value", value: string \| number \| boolean \| undefined\): void;[\s\S]*\(event: "attach-skill", skillKey: string\): void;[\s\S]*\(event: "task-input", inputEvent: Event\): void;[\s\S]*\}>/);
  ...
  assert.match(componentSource, /<textarea[\s\S]*class="node-card__surface node-card__surface-textarea"[\s\S]*:value="body\.taskInstruction"[\s\S]*@input="emit\('task-input', \$event\)"/);
