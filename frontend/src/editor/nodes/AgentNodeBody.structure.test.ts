...
test("AgentNodeBody owns agent body presentation and forwards parent side effects", () => {
  ...
  assert.match(componentSource, /const props = defineProps<\{[\s\S]*nodeId: string;[\s\S]*body: AgentBodyViewModel;[\s\S]*orderedInputPorts: NodePortViewModel\[\];[\s\S]*orderedOutputPorts: NodePortViewModel\[\];[\s\S]*modelOptions: AgentModelOption\[\];[\s\S]*breakpointEnabled: boolean;[\s\S]*selectedActionKey: string;[\s\S]*availableActionDefinitions: ActionDefinition\[\];[\s\S]*\}>/);
  assert.match(componentSource, /defineEmits<\{[\s\S]*\(event: "port-click", anchorId: string, stateKey: string\): void;[\s\S]*\(event: "update:model-value", value: string \| number \| boolean \| undefined\): void;[\s\S]*\(event: "select-action", actionKey: string\): void;[\s\S]*\(event: "update-action-instruction", payload: \{ actionKey: string; content: string \}\): void;[\s\S]*\(event: "task-input", inputEvent: Event\): void;[\s\S]*\}>/);
  ...
  assert.match(componentSource, /<AgentActionPicker[\s\S]*:selected-action-key="selectedActionKey"[\s\S]*:available-action-definitions="availableActionDefinitions"[\s\S]*:breakpoint-enabled="breakpointEnabled"[\s\S]*@update:selected-action="emit\('select-action', \$event\)"[\s\S]*@update:breakpoint-enabled="emit\('update:breakpoint-enabled', \$event\)"/);
  ...
  assert.match(componentSource, /class="node-card__action-instruction-capsule"[\s\S]*@update:model-value="emit\('update-action-instruction'/);
