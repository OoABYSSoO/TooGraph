...
test("AgentRuntimeControls owns agent runtime control presentation and emits parent side effects", () => {
  ...
  assert.match(componentSource, /defineProps<\{[\s\S]*modelValue\?: string;[\s\S]*modelOptions: AgentModelOption\[\];[\s\S]*globalModelRef: string;[\s\S]*thinkingModeValue: AgentThinkingControlMode;[\s\S]*thinkingOptions: AgentThinkingOption\[\];[\s\S]*thinkingEnabled: boolean;[\s\S]*breakpointEnabled: boolean;[\s\S]*confirmPopoverStyle: CSSProperties;[\s\S]*\}>/);
  assert.match(componentSource, /defineEmits<\{[\s\S]*\(event: "model-visible-change", visible: boolean\): void;[\s\S]*\(event: "update:model-value", value: string \| number \| boolean \| undefined\): void;[\s\S]*\(event: "update:thinking-mode", value: string \| number \| boolean \| undefined\): void;[\s\S]*\(event: "update:breakpoint-enabled", value: string \| number \| boolean\): void;[\s\S]*\}>/);
  ...
  assert.match(componentSource, /:class="\{ 'node-card__agent-toggle-card--enabled': breakpointEnabled \}"/);
  assert.match(componentSource, /class="node-card__agent-breakpoint-icon"/);
  assert.match(componentSource, /:class="\{ 'node-card__agent-breakpoint-icon--enabled': breakpointEnabled \}"/);
  assert.match(componentSource, /class="node-card__agent-toggle-switch node-card__agent-breakpoint-switch"/);
