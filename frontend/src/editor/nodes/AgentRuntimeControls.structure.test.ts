...
test("AgentRuntimeControls owns agent runtime control presentation and emits parent side effects", () => {
  ...
  assert.match(componentSource, /<ToographSelect[\s\S]*ref="agentModelSelectRef"[\s\S]*class="node-card__agent-model-select"[\s\S]*:model-value="modelValue"[\s\S]*:placeholder="modelOptions\.length === 0 \? t\('nodeCard\.noConfiguredModels'\) : t\('nodeCard\.selectModel'\)"[\s\S]*:disabled="modelOptions\.length === 0"[\s\S]*popper-class="node-card__agent-model-popper"[\s\S]*@visible-change="emit\('model-visible-change', \$event\)"[\s\S]*@update:model-value="emit\('update:model-value', \$event\)"/);
  ...
  assert.match(componentSource, /<ToographSelect[\s\S]*class="node-card__agent-thinking-select"[\s\S]*:model-value="thinkingModeValue"[\s\S]*popper-class="node-card__agent-thinking-popper"/);
