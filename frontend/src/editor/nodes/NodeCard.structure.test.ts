...
test("ToolNodeBody renders deterministic tool selection with managed ports", () => {
  ...
  assert.match(toolNodeBodySource, /updateObjectStaticInputProperty/);
  assert.match(toolNodeBodySource, /staticInputObjectProperties/);
  assert.match(toolNodeBodySource, /class="tool-node-body__static-panel"/);
  assert.match(toolNodeBodySource, /class="tool-node-body__static-compact-grid"/);
  assert.match(toolNodeBodySource, /class="tool-node-body__limits-grid"/);
  assert.match(toolNodeBodySource, />Card values</);
  assert.match(toolNodeBodySource, /<ToographSelect[\s\S]*class="tool-node-body__static-select"[\s\S]*:model-value="staticInputSelectValue\(field\)"[\s\S]*@update:model-value="\$event => updateSelectStaticInputValue\(field, \$event\)"/);
  assert.match(toolNodeBodySource, /<ElInputNumber[\s\S]*class="tool-node-body__static-number"/);
  assert.match(toolNodeBodySource, /<ElInputNumber[\s\S]*class="tool-node-body__limit-number"/);
  assert.match(toolNodeBodySource, /<ElInput[\s\S]*class="tool-node-body__static-text"/);
  assert.doesNotMatch(toolNodeBodySource, /class="tool-node-body__static-input-row"/);
  assert.doesNotMatch(toolNodeBodySource, /class="tool-node-body__static-mode-switch"/);
  assert.doesNotMatch(toolNodeBodySource, /active-text="Card"/);
  assert.doesNotMatch(toolNodeBodySource, /inactive-text="Port"/);
...
test("NodeCard delegates input body presentation while keeping the output state slot", () => {
  ...
  assert.match(inputSection, /<InputNodeBody[\s\S]*:body="view\.body"[\s\S]*:input-boundary-selection="inputBoundarySelection"[\s\S]*:input-type-options="inputTypeOptions"[\s\S]*:input-asset-envelope="inputAssetEnvelope"[\s\S]*:local-folder-root="localFolderValue\.root"[\s\S]*:local-folder-entries="localFolderPreviewEntries"[\s\S]*:local-folder-selected="localFolderValue\.selected"[\s\S]*:local-folder-selection-mode="localFolderValue\.selection_mode"[\s\S]*:local-folder-hidden-entry-count="localFolderHiddenEntryCount"[\s\S]*@update:boundary-selection="handleInputBoundarySelection"[\s\S]*@local-folder-root-input="handleLocalFolderRootInput"[\s\S]*@local-folder-refresh="handleLocalFolderRefresh"[\s\S]*@local-folder-selection-toggle="handleLocalFolderSelectionToggle"[\s\S]*@local-folder-select-all="selectAllLocalFolderFiles"[\s\S]*@local-folder-clear="clearLocalFolderSelection"[\s\S]*@asset-file-change="handleInputAssetFileChange"[\s\S]*@asset-drop="handleInputAssetDrop"[\s\S]*@clear-asset="clearInputAsset"[\s\S]*@input-value="handleInputValueInput"/);
...
test("NodeCard renders condition nodes as clean control-flow proxies", () => {
  ...
  assert.match(conditionSection, /:condition-rule-value-editor-mode="conditionRuleValueEditorMode"/);
  assert.match(conditionSection, /:condition-rule-value-input-type="conditionRuleValueInputType"/);
  assert.match(conditionSection, /:condition-rule-boolean-value="conditionRuleBooleanValue"/);
  ...
  assert.match(conditionSection, /@rule-boolean-value="handleConditionRuleBooleanValue"/);
  ...
  assert.match(conditionNodeBodySource, /<ElSwitch[\s\S]*conditionRuleValueEditorMode === 'boolean'/);
  assert.match(conditionNodeBodySource, /@update:model-value="emit\('rule-boolean-value', Boolean\(\$event\)\)"/);
  assert.match(conditionNodeBodySource, /:type="conditionRuleValueInputType"/);
  ...
  assert.match(componentSource, /buildConditionRuleEditorModel\(props\.node\.config\.rule, props\.stateSchema\)/);
  assert.match(componentSource, /conditionRuleValueDraft\.value = resolveConditionRuleValueDraft\(ruleValue\);/);
  assert.match(componentSource, /const conditionRuleValueDisabled = computed\(\(\) => conditionRuleEditorModel\.value\?\.isValueDisabled \?\? false\);/);
  assert.match(componentSource, /updateConditionRule\(resolveConditionRuleOperatorPatch\(value\)\);/);
  assert.match(componentSource, /resolveConditionRuleValuePatch\([\s\S]*conditionRuleValueDraft\.value,[\s\S]*props\.node\.config\.rule\.value,[\s\S]*conditionRuleEditorModel\.value\?\.sourceType \?\? "",[\s\S]*\)/);
  assert.match(componentSource, /function handleConditionRuleBooleanValue\(value: boolean\)/);
  assert.match(componentSource, /resolveConditionRuleBooleanValuePatch\(value, props\.node\.config\.rule\.value\)/);
