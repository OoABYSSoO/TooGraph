...
export function createCreativeFactoryGraphDocument(graphId: string, themePresetId?: string): GraphDocument {
  const themePreset = getCreativeFactoryThemePresetById(themePresetId ?? "") ?? CREATIVE_FACTORY_THEME_PRESETS[0];
  const baseNodes = [
