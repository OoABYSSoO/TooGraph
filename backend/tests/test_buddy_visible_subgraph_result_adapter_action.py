...
ADAPTER_ACTION_DIR = REPO_ROOT / "action" / "official" / "buddy_visible_subgraph_result_adapter"
ADAPTER_MANIFEST_PATH = ADAPTER_ACTION_DIR / "action.json"
ADAPTER_AFTER_LLM_PATH = ADAPTER_ACTION_DIR / "after_llm.py"
...
class BuddyVisibleSubgraphResultAdapterActionTests(unittest.TestCase):
    def test_manifest_exposes_internal_result_package_adapter_contract(self) -> None:
        definition = _parse_native_action_manifest(ADAPTER_MANIFEST_PATH, ActionSourceScope.OFFICIAL).definition

        self.assertEqual(definition.action_key, "buddy_visible_subgraph_result_adapter")
    ...
    def test_after_llm_rejects_non_subgraph_capability(self) -> None:
        result = _run_action_script(
            ADAPTER_AFTER_LLM_PATH,
            {
                "selected_capability": {"kind": "action", "key": "web_search", "name": "Web Search"},
