...
SKILLS_ROOT = REPO_ROOT / "action" / "official"


def _load_skill_module(skill_key: str):
    script_path = SKILLS_ROOT / skill_key / "after_llm.py"
    spec = importlib.util.spec_from_file_location(f"test_{skill_key}_after_llm", script_path)
    if spec is None or spec.loader is None:
        raise AssertionError(f"Could not load {skill_key} script.")
...
class GraphTemplateSkillTests(unittest.TestCase):
    def test_reader_returns_existing_official_template_without_runtime_artifacts(self) -> None:
        reader = _load_skill_module("toograph_graph_template_reader")
    ...
    def test_validator_reports_node_system_and_runtime_compatibility(self) -> None:
        validator = _load_skill_module("toograph_graph_template_validator")
    ...
    def test_writer_saves_user_template_with_recoverable_revision(self) -> None:
        writer = _load_skill_module("toograph_graph_template_writer")
