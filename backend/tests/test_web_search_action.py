...
def _load_web_search_module():
    spec = importlib.util.spec_from_file_location("toograph_web_search_action_test", WEB_SEARCH_AFTER_LLM_PATH)
    if spec is None or spec.loader is None:
        raise AssertionError("Could not load web_search action script.")
...
class WebSearchActionTests(unittest.TestCase):
    def test_web_search_action_does_not_mutate_llm_generated_query_with_current_date(self) -> None:
        ...
        with (
            ...
            result = web_search.web_search_action(query="最新模型发布日期")
    ...
    def test_web_search_action_normalizes_tavily_results_when_api_key_is_configured(self) -> None:
        web_search = _load_web_search_module()
        with (
            ...
            result = web_search.web_search_action(
    ...
    def test_web_search_action_falls_back_to_duckduckgo_without_api_key(self) -> None:
        web_search = _load_web_search_module()
        with (
            patch.dict(os.environ, {}, clear=True),
            patch.object(web_search, "_search_with_duckduckgo") as duckduckgo_search,
            ...
            result = web_search.web_search_action(query="fallback search")
    ...
    def test_web_search_action_retries_transient_search_errors_five_times(self) -> None:
        web_search = _load_web_search_module()
        with (
            patch.dict(os.environ, {}, clear=True),
            patch.object(web_search, "_search_with_duckduckgo") as duckduckgo_search,
            ...
            result = web_search.web_search_action(query="retry search")
    ...
    def test_web_search_action_returns_last_error_after_five_failed_search_attempts(self) -> None:
        web_search = _load_web_search_module()
        with (
            patch.dict(os.environ, {}, clear=True),
            patch.object(web_search, "_search_with_duckduckgo") as duckduckgo_search,
    ...
    def test_web_search_action_fetches_pages_to_local_artifacts_when_requested(self) -> None:
        web_search = _load_web_search_module()
                ...
                with (
                    patch.dict(
                        os.environ,
                        {
                            ...
                            "TOOGRAPH_ACTION_ARTIFACT_RELATIVE_DIR": "run_1/searcher/web_search/invocation_001",
                        },
                        clear=True,
                    ),
                    patch.object(web_search, "_search_with_duckduckgo") as duckduckgo_search,
                    ...
                    result = web_search.web_search_action(query="full article")
    ...
    def test_web_search_action_fetches_every_returned_result_to_local_artifacts(self) -> None:
        web_search = _load_web_search_module()
        server = _start_article_server(
            """
            <html>
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with (
                    ...
                    result = web_search.web_search_action(query="full article list")
    ...
    def test_web_search_action_deduplicates_candidate_urls_before_fetching_documents(self) -> None:
        web_search = _load_web_search_module()
        with tempfile.TemporaryDirectory() as temp_dir:
            artifact_dir = Path(temp_dir) / "run_1" / "searcher" / "web_search" / "invocation_001"
            with (
                ...
                result = web_search.web_search_action(query="dedupe candidates")
    ...
    def test_web_search_action_keeps_trying_candidates_until_target_documents_are_saved(self) -> None:
        web_search = _load_web_search_module()
        with tempfile.TemporaryDirectory() as temp_dir:
            artifact_dir = Path(temp_dir) / "run_1" / "searcher" / "web_search" / "invocation_001"
            ...
            with (
                ...
                result = web_search.web_search_action(query="candidate replenishment")
    ...
    def test_web_search_action_retries_transient_page_fetch_errors_five_times(self) -> None:
        web_search = _load_web_search_module()
        with tempfile.TemporaryDirectory() as temp_dir:
            artifact_dir = Path(temp_dir) / "run_1" / "searcher" / "web_search" / "invocation_001"
            with (
                ...
                result = web_search.web_search_action(query="retry page fetch")
    ...
    def test_web_search_action_uses_playwright_when_basic_fetch_has_no_readable_text(self) -> None:
        web_search = _load_web_search_module()
        with tempfile.TemporaryDirectory() as temp_dir:
            artifact_dir = Path(temp_dir) / "run_1" / "searcher" / "web_search" / "invocation_001"
            with (
                ...
                result = web_search.web_search_action(query="rendered page")
    ...
    def test_web_search_action_returns_structured_error_for_missing_query(self) -> None:
        web_search = _load_web_search_module()
        result = web_search.web_search_action(query="   ")
    ...
    def test_web_search_action_is_runtime_registered(self) -> None:
        registry = get_action_registry(include_disabled=True)
