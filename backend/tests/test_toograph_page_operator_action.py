...
PAGE_OPERATOR_ACTION_DIR = REPO_ROOT / "action" / "official" / "toograph_page_operator"
PAGE_OPERATOR_MANIFEST_PATH = PAGE_OPERATOR_ACTION_DIR / "action.json"
PAGE_OPERATOR_BEFORE_LLM_PATH = PAGE_OPERATOR_ACTION_DIR / "before_llm.py"
PAGE_OPERATOR_AFTER_LLM_PATH = PAGE_OPERATOR_ACTION_DIR / "after_llm.py"


def _run_action_script(script_path: Path, payload: dict[str, object], *, env: dict[str, str] | None = None) -> dict[str, object]:
...
class TooGraphPageOperatorActionTests(unittest.TestCase):
    def test_manifest_exposes_page_operator_contract(self) -> None:
        definition = _parse_native_action_manifest(PAGE_OPERATOR_MANIFEST_PATH, ActionSourceScope.OFFICIAL).definition
        ...
        result = _run_action_script(
            PAGE_OPERATOR_AFTER_LLM_PATH,
            {
                "template_target": {
                    ...
                    "input_text": "研究 TooGraph 页面操作Action的最新差距。",
                },
    ...
    def test_after_llm_emits_graph_edit_playback_event(self) -> None:
        ...
        result = _run_action_script(
            ...
            env={"TOOGRAPH_ACTION_RUNTIME_CONTEXT": json.dumps({"page_path": "/editor/new"})},
        )
