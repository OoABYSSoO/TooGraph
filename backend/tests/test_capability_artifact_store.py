...
class ActionArtifactStoreTests(unittest.TestCase):
    def test_create_capability_artifact_context_builds_whitelisted_invocation_folder(self) -> None:
    ...
    def test_read_capability_artifact_text_reads_inside_whitelist_and_rejects_escape_paths(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir) / "capability_artifacts"
            ...
            with patch("app.core.storage.capability_artifact_store.CAPABILITY_ARTIFACT_DATA_DIR", root):
                payload = read_capability_artifact_text("run_1/searcher/doc_001.md")
                with self.assertRaisesRegex(ValueError, "inside the capability artifact folder"):
                    read_capability_artifact_text("../outside.md")
